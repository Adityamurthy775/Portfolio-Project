...
// Education section reveal — same shape as SECTION_REVEAL but with a slightly
...
// left/right reveal feels balanced — long sweep on the right, subtle nudge on the left.
const ITEM2 = {
 ...
 *
 *   title    — credential name (large heading)
 *   issuer   — organization that issued it (eyebrow / subtitle)
 *   status   — 'Completed' | 'Ongoing' — drives the badge color
 *   year     — year issued / expected
 *   summary  — 1–2 sentence description of what was learned
 *   link     — full URL to the credential, or null
 * ──────────────────────────────────────────────────────────────────────────── */
const education = [
  {
    ...
    year: '2023 — 2027',
    summary:
      'Current CGPA 9.1. Coursework spans data science, machine learning, data analytics, and web development.',
    link: null,
  },
]
...
const certifications = [
  {
    ...
    year: '2024',
    summary:
      'Completed Harvard\'s CS50 Python course — covered basic to advanced Python concepts and problem solving.',
    link: '/harvad.pdf',
  },
  {
    ...
    year: '2025',
    summary:
      'Currently in progress. Completed 2 of 3 modules covering ML fundamentals, supervised learning, and best practices.',
  ...
  },
  {
    ...
    year: '2025',
    summary:
      'Completed IBM SkillsBuild AI Fundamentals — covered NLP models, computer vision, and core AI concepts.',
    link: '/ibm.pdf',
  },
  {
    ...
    year: '2026',
    summary:
      'Completed the Google Cloud Data Analytics certificate — cloud fundamentals, the data analyst lifecycle, and hands-on labs.',
    link: 'https://www.credly.com/badges/046b6a6b-b866-4e60-af7c-b6987d914749/public_url',
  },
]
 ...
 *
 * Props:
 *   title    — large uppercase heading (e.g. "AI & DATA")
 *   subtitle — small caption underneath the title
 *   body     — paragraph shown inside the collapsible region
 *   skills   — array of strings rendered as pills
 * ──────────────────────────────────────────────────────────────────────────── */
function ServiceCard({ title, subtitle, body, skills }) {
  ...
  return (
    // Card root — hover handlers live here so the entire card is the hit area.
    <div
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      className='relative rounded-md border border-dashed border-white/25 bg-white/[0.02] px-6 py-7 md:px-8 md:py-9'
    >
      {/* Decorative corner brackets (┌ ┐ └ ┘) — each is a small absolutely-
      ...
      <div className='flex items-start justify-between gap-6'>
        ...
        {/* Visual chevron — rotates on hover to hint at the interaction.
...
function CredentialCard({ title, issuer, status, year, summary, link }) {
  ...
  return (
    ...
    >
      {/* Corner brackets — same visual language as ServiceCard. */}
      <span className='absolute -top-px -left-px h-4 w-4 border-t-2 border-l-2 border-white/70' />
      <span className='absolute -top-px -right-px h-4 w-4 border-t-2 border-r-2 border-white/70' />
      <span className='absolute -bottom-px -left-px h-4 w-4 border-b-2 border-l-2 border-white/70' />
      <span className='absolute -bottom-px -right-px h-4 w-4 border-b-2 border-r-2 border-white/70' />
      ...
      {/* Inline "View Certificate" button — appears only when there's a link.
...
function About() {
  return (
    ...
    >
      ...
      >
        <ScrollVelocity
          texts={['Data Scientist · ML Engineering · Full Stack Developer']}
          velocity={80}
      ...
      >
        ...
        <motion.div variants={ITEM2} className='md:col-span-7 flex flex-col gap-6'>
          <ServiceCard
            ...
            body='Developing AI-powered applications using machine learning, Python, and LLM APIs — with a focus on real-world fraud detection and intelligent automation.'
            skills={aiSkills}
          />
          <ServiceCard
            title='Full Stack'
            subtitle='Scalable Web Architecture'
            body='Building responsive React interfaces and FastAPI backend services. Integrating databases and deploying complete solutions to the cloud.'
      ...
      <div id='education' className='mx-auto max-w-6xl px-6 mt-32 md:mt-40'>
        ...
        {/* Degree card — full width, separate from the cert grid. */}
        <motion.div
          initial='hidden'
          whileInView='visible'
          viewport={{ once: false, amount: 0.3 }}
        ...
        {/* Certifications grid — 1 column on mobile, 2 on tablet+. */}
        <motion.div
          initial='hidden'
          whileInView='visible'
